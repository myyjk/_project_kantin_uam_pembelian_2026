...
$id_user_login = $_SESSION['currentUser']['id_user'];
...
// Validasi nominal
if ($nominal_bayar <= 0) {
    echo "<script>alert('⚠️ Nominal bayar harus lebih dari 0!'); window.history.back();</script>"; exit();
}

// Validasi foto wajib
if (!isset($_FILES['foto_faktur']) || $_FILES['foto_faktur']['error'] !== 0) {
    echo "<script>alert('⚠️ Foto bukti pembayaran wajib diupload!'); window.history.back();</script>"; exit();
}

// Cek kepemilikan & hitung sisa hutang
$cek = mysqli_query($conn, "SELECT 
    ...
    WHERE p.id_beli = '$id_beli' AND p.id_admin = '$id_user_login'
    GROUP BY p.id_beli");

$data = mysqli_fetch_assoc($cek);

if (!$data) {
    echo "<script>alert('⚠️ Akses ditolak atau data tidak ditemukan!'); window.location='piutang.php';</script>"; exit();
}
...
if ($sisa_hutang <= 0) {
    echo "<script>alert('⚠️ Hutang sudah lunas!'); window.location='piutang.php';</script>"; exit();
}

// Jangan boleh bayar melebihi sisa
if ($nominal_bayar > $sisa_hutang) $nominal_bayar = $sisa_hutang;
...
if (!in_array($ext, $allowed)) {
    echo "<script>alert('⚠️ Format foto harus JPG, PNG, atau WEBP!'); window.history.back();</script>"; exit();
}

$nama_file = "bayar_{$id_beli}_" . time() . ".$ext";
if (!move_uploaded_file($_FILES['foto_faktur']['tmp_name'], $target_dir . $nama_file)) {
    echo "<script>alert('⚠️ Gagal mengupload foto!'); window.history.back();</script>"; exit();
}
...
if (mysqli_query($conn, $insert)) {
    ...
    $pesan = $sisa_setelah <= 0 
        ...
        : '✅ Pembayaran berhasil dicatat! Sisa hutang: Rp ' . number_format($sisa_setelah, 0, ',', '.');
